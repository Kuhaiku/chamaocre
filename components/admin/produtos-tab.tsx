...
import { Loader2, Plus, Trash, Edit2, X, Image as ImageIcon, UploadCloud, ShoppingBag, Save, ArrowLeft } from 'lucide-react'
...
export function ProdutosTab() {
  ...
  return (
    <div className="animate-fade-in">
      {!isFormAberto ? (
        ...
        <div className="bg-white rounded-lg shadow-sm border border-stone-200 w-full flex flex-col relative animate-fade-in">
          
          <div className="px-5 py-4 border-b border-stone-200 flex items-center justify-between bg-white shrink-0">
            <div className="flex items-center gap-4">
              <button onClick={() => setIsFormAberto(false)} className="p-1.5 text-stone-400 hover:text-stone-700 hover:bg-stone-100 rounded-md transition-colors outline-none">
                <ArrowLeft size={20} />
              </button>
          ...
          <form id="form-produto" onSubmit={handleSalvarProduto} className="p-5 sm:p-6 bg-white">
  ...
  {imagens.map((img, idx) => (
    <div 
      ...
      // Medidas travadas em 60px aqui:
      className={`relative min-w-[60px] max-w-[70px] min-h-[60px] max-h-[60px] shrink-0 rounded-sm border-2 overflow-hidden bg-white cursor-grab active:cursor-grabbing group ${idx === 0 ? 'border-[#C87A2C]' : 'border-stone-200'}`}
    >
      <img 
        src={img.url} 
        alt={`Preview ${idx}`} 
        // absolute e inset-0 impedem a imagem de empurrar o container
        className="absolute inset-0 w-full h-full object-cover" 
      />
      <button 
        type="button" 
        onClick={() => handleRemoveImage(idx)} 
        className="absolute top-0 right-0 m-0.5 p-0.5 bg-red-500 text-white rounded-sm opacity-0 group-hover:opacity-100 transition-opacity z-10"
      >
        <X size={10} />
      </button>
    </div>
