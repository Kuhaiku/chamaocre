...
import { Loader2, Plus, Trash, Edit2, X, Image as ImageIcon, GripVertical, UploadCloud } from 'lucide-react'
...
export function ProdutosTab() {
  ...
  const handleSalvarProduto = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Fazer upload das novas imagens primeiro
      ...
      // 2. Prepara o payload final
      const payload = {
        ...produtoEditando,
        image: imagensParaSalvar.length > 0 ? imagensParaSalvar[0].url : '', // A primeira é a capa
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
          {produtos.map(p => (
            <div key={p.id} className="bg-white border border-stone-200 rounded-md shadow-sm overflow-hidden flex flex-col">
              ...
              {/* BOTÕES REAIS NO RODAPÉ DO CARD */}
      ...
      {isFormAberto && produtoEditando && (
        <div className="fixed inset-0 bg-stone-900/80 backdrop-blur-sm z-[80] flex items-center justify-center p-2 sm:p-4" onClick={(e) => { if (e.target === e.currentTarget) setIsFormAberto(false) }}>
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-4xl max-h-[95vh] flex flex-col relative overflow-hidden">
            ...
            <form id="form-produto" onSubmit={handleSalvarProduto} className="flex-1 overflow-y-auto p-5 sm:p-6 bg-white">
              ...
              {formTab === 'basico' && (
                <div className="space-y-6">
                  ...
                  {/* UPLOAD E REORDENAÇÃO DE IMAGENS */}
                    ...
                    <div className="flex flex-wrap gap-3">
                      {imagens.map((img, idx) => (
                        <div 
                          ...
                          className={`relative w-24 h-24 rounded-md border-2 overflow-hidden bg-white cursor-grab active:cursor-grabbing group ${idx === 0 ? 'border-[#C87A2C]' : 'border-stone-200'}`}
                        >
                          <Image src={img.url} alt={`img-${idx}`} fill className="object-cover" />
              ...
              {formTab === 'frete' && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    <label className="text-xs font-bold text-stone-900 uppercase mb-1.5 block">Peso Bruto (Kg) *</label>
                    <input required type="number" step="0.01" value={produtoEditando.weight} onChange={(e) => setProdutoEditando({...produtoEditando, weight: e.target.value})} className="w-full px-3 py-2.5 bg-white border border-stone-300 rounded-md text-sm text-stone-900 focus:border-[#C87A2C] focus:ring-1 focus:ring-[#C87A2C] outline-none" />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-stone-900 uppercase mb-1.5 block">Altura (cm) *</label>
                  ...
                  <div>
                    <label className="text-xs font-bold text-stone-900 uppercase mb-1.5 block">Largura (cm) *</label>
                  ...
                  <div>
                    <label className="text-xs font-bold text-stone-900 uppercase mb-1.5 block">Comprimento (cm) *</label>
